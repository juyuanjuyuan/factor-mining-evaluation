...
行业中性化暂缺行业分类数据，未做。已知风险：第 2 步用原始 IC 先淘汰，
可能误杀"alpha 被市值暴露反向掩盖"的因子；两步计算都便宜，后续可改为
并行跑两版、以中性化 IC 为主要淘汰依据。
...
- 可交易性过滤：入场日 `t+1` 开盘触及涨跌停代理边界
  `abs(open[t+1] / close[t] - 1) >= limit[t+1] - 0.002`，或入场日为
  ST/*ST 的标的，在因子第 `t` 行置 NaN。这样覆盖“开盘封板后盘中打开”的
  入场不可成交情形；仍不查出场日。
