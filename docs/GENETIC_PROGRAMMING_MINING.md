...
每一代训练期的有符号 Rank IC 若为负，候选树会先同向化为其相反数，再参与锦标赛、交叉、
变异、精英保留和 Hall of Fame；表达式冻结后才进入测试集。测试集不参与进化、超参数选择、
候选去重或因子方向调整。
...
- 锦标赛选择，默认每次抽取 20 个程序；
- 交叉、子树替换、删除/插入节点、单节点替换、窗口参数变异、常数扰动、Hoist
  变异与随机新树；普通复制不再作为遗传通道，精英复制仅由 `elite_size` 负责；
- 默认每代 1000 个公式、进化 3 代；
- 训练适应度保留逐日横截面 Rank IC 均值的有符号诊断；每代负值候选会仅依训练集改写为
  相反数，再按同向化后的 IC 均值和最终表达式长度惩罚排序，并将该正向树作为下一代父代；
  同向化后相同的树在锦标赛父代池中去重，不会因原始正负镜像而获得重复抽样机会；
- 父代锦标赛仍使用 `abs(IC) - parsimony_coefficient * node_count`；跨代 Hall of Fame
  独立改为“同向训练 IC 最大化、节点数最小化”的 Pareto 存档。同一节点数只保留 IC
  最强者；前沿超过容量时同时保护最高 IC 候选并按复杂度覆盖裁剪；
- 从 Pareto Hall of Fame 按复杂度覆盖冻结最多 `n_components` 个候选进入独立测试集。
  HOF 内不做两两相关性筛除：先以测试集盈利能力筛掉大部分候选，
  仅让存活者做测试集 IC 检测；两关均通过后自动交接给因子库服务。因子库负责
  相关性检验及正式入库裁决；GP 不执行相关性检验，也不直接写入正式因子库。
- 表达式在初始生成、每次遗传操作和训练期方向同向化后执行保守规范化：消除双重负号、
  加零/乘一等恒等结构，折叠安全常数子树，并稳定排序 `add`/`mul` 子树。不会执行
  `x/x -> 1`、任意树旋转等可能改变 NaN、浮点或保护除法语义的激进化简。
- 默认三代复杂度热身依次使用 `depth/nodes = 4/31、6/63、8/127`；生成下一代时使用
  下一代上限，因此最后一代确实能够探索完整配置空间。单代任务或
  `--no-complexity-warmup` 从一开始使用最终上限。
...
- 训练适应度剔除测试项目里同口径的信号日/买入日 ST、退市整理期、
  涨跌停开盘不可交易观测，并以买入日 `amt` 有限且严格为正剔除停牌/零成交观测。
...
这里使用净分组收益：买入按 12.5 bps、卖出按 17.5 bps 计费；一单位完整买卖的
成本为 30 bps。分段夏普使用完整、非重叠的 60 交易日区块。最高组年化收益按测试
样本内的最高组净日收益几何年化。
...
相关性检验。交接单同时携带冻结测试集上的盈利能力结果，因此若候选与正式库因子相关性超过
阈值，因子库先比较双方同口径的 60 日窗口 Sharpe 中位数；并列才比较 Fitness。候选必须严格优于
每一个冲突因子才能替换它们，否则保留在测试库。缺少同口径结果时，因子库会为双方补跑“盈利能力
测试”流水线并将结果写入 Webapp 运行历史；替换只移除旧的正式库副本，不删除源测试定义。独立标准
CLI 的 `--admit-if-passed` 要求 `ic_test` 与 `profitability_test` 都通过。正式因子库沿用现有契约：所有
对齐、有限的日期-证券暴露上的 pooled Pearson，任一非对角元素绝对值不得超过 0.75。通过后只向
`factor_registry/webapp_factor_library.json` 写因子定义；评价指标继续保存在输出目录，避免污染注册表。

Hall of Fame 先按训练期同向 IC 最大化、表达式节点数最小化执行多层 Pareto 排名。第一前沿
优先，若不足配置的冻结测试候选数，则依次从第二、第三及后续前沿补足；只有有效且规范化去重
后的表达式总数不足时，冻结数量才会少于配置值。这样 Pareto 排名决定测试优先级，但不会仅因
训练 IC/复杂度被支配就永久丢弃可能具有样本外盈利能力的候选。

该两阶段筛选、逐代训练期 IC 基因型同向化、多层 Pareto HOF、扩充变异、复杂度热身且不自动
入库的 v8 协议会写入 campaign 配置指纹。v8 的 `paper_local` 可交易性预处理同时
使用信号日与买入日的正式 ST/*ST、退市整理期状态，并保留买入日开盘涨跌停与
全日成交额为正的执行检查。后者是事后可行性代理，会进入 `paper_local` 适应度口径，
不应被解释为信号日可见的因子输入。
已有的 v7 及更早 campaign 不会被静默改写或
续跑；要采用本协议，应新建一个 campaign 名称，以保留既有训练/测试结果的审计含义。
