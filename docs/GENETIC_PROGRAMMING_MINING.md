...
- 三代训练结果合并为 Hall of Fame（默认保留前 100 个）；再按训练期 fitness 直接冻结前
  ...
  仅让存活者做测试集 IC 检测；两关均通过后，才逐个进入正式因子库的增量相关性准入。
...
GP 自动入库时，先通过盈利能力标准、再通过 IC 标准，才运行正式因子库相关性检验。独立标准 CLI 的
`--admit-if-passed` 同样要求 `ic_test` 与 `profitability_test` 都通过。正式因子库
沿用现有契约：所有对齐、有限的日期-证券暴露上的 pooled Pearson，任一非对角元素绝对值
不得超过 0.75。通过后只向 `factor_registry/webapp_factor_library.json` 写因子定义；评价指标
继续保存在输出目录，避免污染注册表。

该两阶段筛选协议会写入 campaign 配置指纹。已有的旧协议 campaign 不会被静默改写；要采用
本协议，应新建一个 campaign 名称，以保留既有训练/测试结果的审计含义。
...
- 是否在盈利标准、存活者 IC 检测与相关性检验均通过后自动写入正式因子库；
